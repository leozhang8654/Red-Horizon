...
## 干活方式（重要）
- **Codex 直接改文件**（用读写/编辑工具改 `.gd`、`.tscn`，需要处理图片就用 Python/Pillow）。
...
> ⚠️ **要移动/重命名文件，务必连同它的 `.import`(图片/音频)或 `.uid`(脚本/着色器)小尾巴一起动**，并更新所有引用它的 `res://` 路径；最稳妥是直接在 Godot 的 FileSystem 面板里拖动(它会自动改引用)。批量整理可由 Codex 用脚本(移动 + 全局改 `res://` 引用 + 改 `.import` 的 `source_file` + 清 `.godot/imported` + 断引用体检)在外部完成，完成后我退出再重开 Godot 让它重新导入。
